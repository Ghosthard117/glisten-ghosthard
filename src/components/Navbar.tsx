...
export default function Navbar({ settings }: NavbarProps) {
  ...
  return (
    ...
    >
      <div
        className="mx-auto flex max-w-6xl flex-col justify-between py-2 font-medium text-white md:flex-row md:items-center"
      >
        <div
            ...
            {settings.data.navigation.map(item => {
              ...
              return (
                <PrismicNextLink
                  key={item.label}
                  field={item.link}
                  className="bloc px-3 text-3xl first:mt-8"
                  onClick={() => setOpen(false)}
                  aria-current={
                    pathname.includes(asLink(item.link) as string)
                      ? "page"
